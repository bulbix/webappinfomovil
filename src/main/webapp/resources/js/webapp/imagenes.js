...
$(document).ready(function() {
	...
	$("#llamarGaleriaImagenes").click(function() {
		...
		if(noEsDispositivo)$("#btnSeleccionaImagen2").show();
...
function isDevice() {  
	if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
		...
		}else{$("#btnSeleccionaImagen2").show(); 
...
{	
	if(IMAGENESDELUSUARIO < IMAGENESMAX){
		...
		convertImgToBase64F(imageUrl, function(base64Img) {
...
function uploadImage(imageDom, imageUrl, origin, textFoto) {
	...
	convertImgToBase64(imageDom, imageUrl, function(base64Img) {
		binaryString = base64Img.replace(/^data:image\/(png|jpeg|jpg);base64,/, "");
		var weHaveSuccess = false;
		$.ajax({
			type : "POST",
...
{	
	...
	if(url.substring(0, 5) == 'data:') {
		var dataURL = resize1();
		console.log('guardar imagen Tamaños (original, redimensionado)', url.length, dataURL.length);
		...
		img.onload = function() {
			var dataURL = resize2();
			console.log('Tamaños (original, redimensionado)', url.length, dataURL.length);
			imageDom.src = dataURL;
			callback(dataURL);
			canvas = null;
		};
...
function logueoFacebook(response)
{
	...
	if (response.status === 'connected') {
		...
		for (var p in infoAlbumes) 
		{
			...
			$li.append('<span class="col-xs-4"><img src="' + infoAlbumes[p].picture + '" style="max-width:90px; max-height:80px;"  class="img-thumbnail"/></span>');
			...
			$li.click(function() {
				...
				for (var a in fotos ) 
				{
					...
					$li.append('<span class="col-xs-12"><img src="' + fotos[a].origen + '" style="max-width:100px; max-height:80px;" class="img-thumbnail"/></span>');
		...
		if( navigator.userAgent.match('CriOS') ){
			//alert('https://www.facebook.com/dialog/oauth?client_id=422690184604551&redirect_uri='+ document.location.href +'&scope=email,public_profile,user_photos');
...
function logueoFacebook2() 
{
	...
	for (var p in infoAlbumes) 
	{
		var $photosList = $('#albumsList');
		var $li = $('<li class="albumDinamico" style="display:block; height:100px; margin:0 5px 5px 5px;"/>');
		$li.append('<span class="col-xs-4"><img src="' + infoAlbumes[p].picture + '" style="max-width:90px; max-height:100px;"  class="img-thumbnail"/></span>');
