...
function logueoFacebook(response) {
	...
	if (response.status === 'connected') {
		...
		if (navigator.userAgent.match('CriOS')) {
			// alert('https://www.facebook.com/dialog/oauth?client_id=422690184604551&redirect_uri='+
			// document.location.href
			// +'&scope=email,public_profile,user_photos');
			window.open(
					'https://www.facebook.com/dialog/oauth?client_id=422690184604551&redirect_uri='
							+ document.location.href
							+ '&scope=email,public_profile,user_photos', '',
					null);
		} else {
